...
These agents are built using the Google ADK and serve as orchestrators for specific tasks within the Literary Companion. Unlike some `cie_core` agents that coordinate other agents, these Literary Companion agents primarily coordinate complex Python functions exposed as tools.
...
*   **`translation_tool.py`:**
    ...
    *   **Key Functions/Tools:**
        *   `translate_text(text)`: Takes a string of text. Uses a generative AI model (Vertex AI, via `DEFAULT_AGENT_MODEL` from `cie_core.config`) to perform a stylistic translation/rephrasing. It's not translating between different languages but rather modernizing the English style. Used by `process_and_translate_book`.
...
*   **`fun_fact_micro_task_board_tool.py`:**
    ...
    *   This board is distinct from the `AgentStatusBoard` in `cie_core` and serves a more fine-grained, internal purpose for the `fun_fact_orchestrator_tool`.
...
*   **Firestore (`fun_fact_micro_task_board` collection):**
    ...
    *   **Separation:** This Firestore collection is *separate and distinct* from the `AgentStatusBoard` (`cie_core_agent_status_board` collection) used by the `cie_core` agents. The Literary Companion's fun fact generation does not use the `cie_core` status board; it has its own specialized board for its internal orchestration logic.
...
The Literary Companion module leverages several key Python libraries and Google Cloud technologies, some shared with `cie_core` and some specific to its needs:
...
*   **Google Cloud Vertex AI:** Directly used by `literary_companion.lib.fun_fact_generators.py` and `literary_companion.tools.translation_tool.py` to interact with generative AI models (specified by `DEFAULT_AGENT_MODEL` from `cie_core.config`) for generating fun facts and modern English translations. This is a more direct use of the models compared to `cie_core` agents which primarily have AI capabilities embedded within the ADK agent framework itself.
